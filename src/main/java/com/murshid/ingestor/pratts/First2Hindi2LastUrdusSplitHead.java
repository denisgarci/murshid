...
public class First2Hindi2LastUrdusSplitHead {

    private static final Logger LOGGER = LoggerFactory.getLogger(First2Hindi2LastUrdusSplitHead.class);
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (IngestorWordUtils.scriptType(tokens[0]) != Scripts.NASTALIQ && IngestorWordUtils.scriptType(tokens[1]) != Scripts.NASTALIQ
                    || IngestorWordUtils.scriptType(tokens[2]) != Scripts.DEVANAGARI || IngestorWordUtils.scriptType(tokens[3]) != Scripts.DEVANAGARI){
                    continue;
                }
