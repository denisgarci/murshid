...
public class PrattsCaller {

    private static final Logger LOGGER = LoggerFactory.getLogger(PrattsCaller.class);

    private static String PRATTS_URL = "http://dsalsrv02.uchicago.edu";
    //?c.8:1:226.platts"
    ...
    public static void findBoundariesForAll(String[] args) throws Exception{
        //new PrattsCaller().invokeAll();
        PrattsCaller prattsCaller = new PrattsCaller();
        for (int section=0; section<=9; section++){
            int noHasBoundary = prattsCaller.findNotHasBoundary(section);
            int lastEntry =  prattsCaller.findEnd(section, 1, noHasBoundary);
    ...
    public static void main(String[] args) throws Exception{
        ...
        for (int i =0; i < 20; i++ ) {
            ...
            URL url = prattsCaller.createUrlWithParams(1, 100 + i);
    ...
    public URL createUrlWithParams(int section, int entry) {
        ...
        try {
            URIBuilder builder = new URIBuilder(PRATTS_URL);
            String relativePath = "/cgi-bin/philologic/getobject.pl";
            builder.setPath(relativePath);
            builder.setCustomQuery("c." + section + ":1:" + entry + ".platts");
