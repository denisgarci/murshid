...
public class FirstUrduLastUrduSplitHead {

    private static final Logger LOGGER = LoggerFactory.getLogger(FirstUrduLastUrduSplitHead.class);
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (WordUtils.scriptType(tokens[0]) != Scripts.NASTALIQ
                        || WordUtils.scriptType(tokens[tokens.length-1]) != Scripts.NASTALIQ){
                    continue;
                }
            ...
            System.out.println("finished " + i + " records");
            con.commit();
