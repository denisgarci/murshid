...
public class BahriCaller {

    private static final Logger LOGGER = LoggerFactory.getLogger(BahriCaller.class);
...
//            .put(9, 11582).build();
    ...
    public static void findBoundariesForAll() {
        ...
        BahriCaller plattsCaller = new BahriCaller();
    ...
    private boolean hasInfo(int section, int entry){
        try {
            ...
            String entity = response.readEntity(String.class);
            Document document = Jsoup.parse(entity);
            Element element = document.selectFirst("hw");
            return element != null;
    ...
    public URL createUrlWithParams(int section, int entry) {
        ...
        try {
            ...
            builder.setCustomQuery("c." + section + ":" + entry + ".bahri");
            return builder.build().toURL();
        } catch (URISyntaxException | MalformedURLException e) {
            String msg = String.format("Error creating URL for Bahri ingestion with parameters %d %d. The exception is %s ", section, entry, e.getMessage());
